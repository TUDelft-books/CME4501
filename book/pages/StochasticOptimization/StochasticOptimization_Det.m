function StochasticOptimization_Det
clc, clear, close all
...
data=[
    ...
    1 7 15 1    
    1 10 20 0.7];
[beta0_b, d_b, L_b, C_b]=deal(data(:,1), data(:,2), data(:,3), data(:,4));
...
beta_bt0=reliability(beta0_b,d_b,L_b,Thoriz);  %reliability with time

%--- figure ----
figure, hold on
col=['r','b','g','m'];
for b=1:nb
    plot(1:Thoriz,beta_bt0(b,:),['-',col(b)],'LineWidth',2)  
end
grid on, xlim([1,Thoriz]), ylim([0,1]), xlabel('Time, t (years)'), ylabel('Reliability index, \beta')
title('Reliability Index Over Time');
%--- ------ ----

%% Deterministic Optimization
%--- Variables ->  e.g., freq=[10 2 0 0 ; 5, 5 0 0; 0 0 0 0];  
maxInter_b=ceil(Thoriz./L_b)+1; %max number of interventions expected per bridge
...
pos=1;
for b=1:nb
    % This guarantees interventions freq. within the service life (if
    % conducted)
    ub(pos:pos+maxInter_b(b)-1)=L_b(b)-1;
...
beta_bt=MaintenanceApplication(beta_bt0,X_bt,Thoriz); %optimal maintenance 
...
[c,~] = nonlcon(Yopt);
BudgetIssues=length(find(c(end-Thoriz:end-1)>0));
...
for b=1:nb
    h(b)=plot(1:Thoriz,beta_bt(b,:),['-',col(b)],'LineWidth',2);  
